...
# Displaying the current size of the screen

def display_screensize():
    print(pyautogui.size())

# this is the class of the mouse movement
class MouseMove:
    x = 0  # initializing the x axis with the zero
    y = 0  # initializing the y axis with zero

    def __init__(self):
        pyautogui.PAUSE = 1  # set the pause equals to one
        pyautogui.FAILSAFE = True  # Sets the Fail safe to be one
        self.display_screensize()  # Printing the size of the display
        self.x, self.y = self.get_positions()
        print(self.x)  # Debuggin purpose
        print(self.y)  # Debuggin purpose
    # This is the Display Screen size


    def display_screensize(self):
        print(pyautogui.size())  #
